...
- DuckDB driver must be the latest at `github.com/duckdb/duckdb-go/v2` and not `github.com/marcboeker/go-duckdb` It does use CGO.
...
### UI
  - Terminal UI uses **Bubble Tea** (`github.com/charmbracelet/bubbletea`).
  - Keep UI logic separate from application/domain logic.
	- Bubble Tea UI Rules
...
### Persistence
   - by default, state is stored in **SQLite**.
	 - only when requested by user, support **DuckDB** as an alternative (for analytics or large datasets). 
   ...
   - On updates: database schema is **versioned** and **migrated** if the stored version differs.
	 - Use **one authoritative schema version integer** and enforce it at startup.

#### Schema Versioning
Acceptable approaches (choose one and stick to it):
- **Preferred for SQLite**: `PRAGMA user_version` as the schema version store.
- Alternative: a dedicated `schema_migrations` table with a single row.


#### Location and Configuration
- DB path must be configurable (flag or env var) and documented in README.
- On startup:
...
#### Migrations
	...
	- Migrations must be **idempotent-safe** where feasible (or guaranteed by version gate).
	- Apply migrations in a **transaction** where SQLite allows.
	- On success, update schema version to the new value.

#### Migration Storage
- Store migrations in-repo under: `internal/db/migrations/`
- Use numbered files, e.g.:
  - `001_init.sql`
  - `002_add_inventory.sql`
- Embed them in the binary using `embed` to avoid runtime file dependencies.

#### Tests for Migration Logic
- Write tests that:
  - Start from an empty DB and reach latest schema.
  - Start from an older schema version and migrate to latest.
  - Verify version number changes appropriately.
- Tests must use a temporary DB file (not in-repo) to mimic real behavior.

#### SQLite Database Drivers
	- SQLite driver must be **pure Go** (e.g. `modernc.org/sqlite`), not `mattn/go-sqlite3` (CGO).
  - SQLite allows only one writer at a time; design accordingly.
  - Ensure builds pass with `CGO_ENABLED=0`.

#### DuckDB Database Drivers
	- DuckDB driver must use latest version (e.g. `github.com/duckdb/duckdb-go`), not `github.com/marcboeker/go-duckdb`.
	- DuckDB driver uses CGO. Ensure builds pass with `CGO_ENABLED=1`.
  - DuckDB allows only one writer at a time; design accordingly.
	- if DuckDB is enabled, ensure it is properly configured and tested in CI with CGO enabled. Also document where you will remember that CGO is required for DuckDB in README and build scripts.
