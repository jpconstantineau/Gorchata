...
WITH all_tests AS (
  ...
  FROM int_velocity_vectors v
  LEFT JOIN int_trip_segments t ON v.trip_segment_id = t.trip_segment_id
  ...
  FROM (
    SELECT origin_location_id AS location_id FROM int_velocity_vectors
    UNION
    SELECT destination_location_id AS location_id FROM int_velocity_vectors
  ) v
  ...
  SELECT
    ...
    CASE 
      WHEN (SELECT COUNT(*) FROM int_velocity_vectors) != 
           (SELECT COUNT(*) FROM int_trip_segments WHERE origin_location_id != destination_location_id)
      THEN 1
      ELSE 0
